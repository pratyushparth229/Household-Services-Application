...
def login():
    ...
    if request.method == "POST":
        ...
        if user and user[2]==password:
            ...
            elif role == ROLE["SERVICE_PROFESSIONAL"] and user[4]!="IN_PROGRESS":
                login_user(User(user[0], user[1], user[3], user[4]))
...
def admin_dashboard():
    ...
    with sqlite3.connect(DATABASE) as conn:
        ...
        cursor.execute(
            "SELECT id, username, approval FROM users WHERE role = 1 AND approval = ?",
            ("IN_PROGRESS",),
        )
        unapproved_professionals_tuples_list = cursor.fetchall()
        unapproved_professionals = [
            {"id": tpl[0], "username": tpl[1], "approval":tpl[2].capitalize()}
    ...
    with sqlite3.connect(DATABASE) as conn:
        ...
        if s:
            user = {
                ...
                "experience": s[2],
                "service_name": s[3],
                "approval": s[4].capitalize(),
                "cv_path":s[5]
            }
            if user['approval']=="In_progress":
                user['approval']="Pending"
            elif user['approval']=='Success' or user['approval']=='Unblocked':
...
def admin_summary():
    ...
    if "customer" not in temp:
        temp["customer"]=0
    if "professional" not in temp:
        temp["professional"]=0
...
def edit_request(request_id):
    ...
    if request.method == "POST":
        ...
        cursor.execute("""SELECT r.id, s.service_name, s.service_category, r.description
                ...
                WHERE r.customer_id = ? and r.id=?""", (customer_id,request_id))
...
def generate_charts(labels_role, values_role, labels_request, values_request):
    ...
    try:
        ...
        plt.title('User Counts by Roles')
